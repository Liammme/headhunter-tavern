...
开发和线上都应使用 PostgreSQL，但不能共用同一个数据库实例。

- 本地开发库：使用本机 PostgreSQL，例如 `bounty_pool_dev`。
- 线上正式库：继续使用服务器 PostgreSQL，承载真实产品数据。
- SQLite：只保留为轻量备用或单元测试环境，不作为市场情报层的主要开发数据库。

这样可以让本地和线上使用同一种数据库行为，减少 SQLite 本地通过、PostgreSQL 上线出错的风险；同时本地开发不会直接写入或污染线上正式数据。

本地 `.env` 应指向本地 PostgreSQL，不应指向线上正式 `DATABASE_URL`。如果需要排查线上数据，后续可以单独配置只读账号，只允许查询，不允许写入、建表或删除。

## 数据分层

继续使用同一个产品数据库中的不同表做分层。线上是 PostgreSQL，本地开发也应使用本机 PostgreSQL，但两者是不同数据库。数据拆成两层生命周期不同的表：
...
- 首页能读取最新成功快照。
- 本地 PostgreSQL 能完成建表、快照写入、首页读取和 `daily_bounty` smoke test。
- SQLite 单元测试通过不能单独作为上线依据；涉及新表或 JSON payload 时，必须补 PostgreSQL 验证。
...
第一阶段数据库结构应尽量只新增 `market_intelligence_snapshots` 表，不修改已有表结构。当前项目没有正式 migration 系统，新增表可以沿用 `init_db()` / `Base.metadata.create_all()`；如果后续需要修改已有表字段、类型、约束或索引，必须先补迁移方案和回滚方案。
