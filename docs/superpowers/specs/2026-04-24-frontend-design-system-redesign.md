...
本轮第一份外部参考为用户提供的 21st.dev / Symbiosis 页面：

- `C:\Users\Administrator\Desktop\Symbiosis___Cross_Chain_Bridge__Swap___Liquidity_Protocol_for_Seamless_DeFi.html`

从源码提取到的视觉特征：

- 整体是明亮产品站气质：白底、黑字、浅灰边界、低噪音背景。
- 字体以 `Geist` 和 `Geist Mono` 为核心，标题和数字清晰、现代、克制。
- 主色非常收敛：`#111111`、`#ffffff`、`#656565`、`#f8f8f8`、`#d4d4d4` 为主，少量链/token 色作为强调。
- 组件圆角稳定，常见值集中在 `24px`、`20px`、`16px`、`12px`。
- 阴影极轻，多数面板只用浅边框、浅阴影和留白建立层级。
- 信息组织偏模块化：产品卡、数据卡、路线卡、新闻卡、生态卡分区明确。
- 数字指标使用等宽字体，形成可信的数据产品感。

可迁移到本项目的方向：

1. 从当前较强的“侦探酒馆 / 手绘纸张”收敛为更现代的明亮产品系统。
2. 保留“赏金猎人 / 情报 / 公司档案”的语义，但减少歪斜、剪角、硬阴影和旧纸色。
3. 用浅色 surface、清晰边框、稳定圆角和精确间距建立产品可信感。
4. 将赏金、岗位数、公司数等关键数据改用数字化、等宽化的视觉语言。
5. 使用少量高识别强调色承载“赏金 / 已认领 / 高价值线索”，避免全页面多色装饰。

不照搬的部分：

- 不复制 DeFi / bridge / token route 的业务布局。
- 不做纯营销 landing page。
- 不把首页改成产品宣传页；首页仍必须服务于每日猎单工作流。
- 不引入额外网络资源、遥测或第三方脚本。
