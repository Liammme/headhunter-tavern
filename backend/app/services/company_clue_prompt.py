...
def build_company_clue_system_prompt() -> str:
    return (
        ...
        "clue_1 标题固定为“线索一：露出的口子”，写这家公司在 14 天岗位窗口里哪里不对劲。"
        "clue_2 标题固定为“线索二：卡住的岗位”，必须点名 evidence_cards 里的具体岗位标题，并说明每个岗位暴露的需求。"
        "clue_3 标题固定为“线索三：下手路径”，给出 2-3 个猎头摸排动作，每个动作必须绑定真实入口。"
...
def build_company_clue_user_prompt(context: dict) -> str:
    return (
        ...
        "clue_3 写猎头下手路径：2-3 个动作，每个动作必须绑定真实 job_post、hiring_page、company_url 或 email。"
        "如果没有公司官网或邮箱，就只允许引用 job_posts。"
        "sections 的标题固定写成“线索一：露出的口子”“线索二：卡住的岗位”“线索三：下手路径”。"
        f"\n\n结构化输入：\n{json.dumps(context, ensure_ascii=False)}"
    )
