...
def request_structured_json(messages: list[dict], *, timeout_seconds: int | None = None) -> str:
    errors: list[str] = []
    for model_name in iter_llm_models():
        try:
            return request_chat_completion_with_model(messages, model_name, timeout_seconds=timeout_seconds)
...
def request_chat_completion_with_model(
    messages: list[dict],
    model_name: str,
    *,
    ...
    try:
        timeout = timeout_seconds if timeout_seconds is not None else settings.bounty_pool_llm_timeout_seconds
